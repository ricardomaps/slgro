...
.new_screen = new_screen, .new_window = new_window, .new_device = new_devic
e,
};
...
wm.sel_screen = wl_container_of(wm.screens.next, wm.sel_scr
een, link);
}
...
/* slgro - im sure i could have made this better but atleast this works rn so ig th
at's what matters,, ill work on simplifying this another time,.,.*/
...
/* slgro - same thing here, ill figure out how to simplify this into one function a
nother time, just focusing on making this work to begin with for now*/
...
/* slgro - idk if this is THAT useful but its kinda useful for me so that's what ma
tters i suppose,,*/
...
if (swc_window_get_geometry(wm.sel_client->win, &geom)) {
geom.x = (wm.sel_client->scr->scr->usable_geometry.width / 2) - (ge
om.width / 2);
geom.y = (wm.sel_client->scr->scr->usable_geometry.height / 2) - (g
eom.height / 2);
swc_window_set_geometry(wm.sel_client->win, &geom);
}
}
